...
void BMP24::LoadFile ( string &filename )
{
	m_Filename = filename;
		...
		if ( m_fstFileInput.is_open ( ) && m_bHeadersLoad )
		{
			if ( m_sInfoHeader.biBitCount == bit_depth_24 && m_sInfoHeader.biCompression == BI_RGB )
			{
				...
				for ( auto i = 0; i < m_sInfoHeader.biHeight; i++ )
				{
					...
					{
						m_matData.set ( i ,j ,static_cast< rgbD >( pStringBuffer[ j ] ) );
					}
				}
#ifdef _DEBUG
...
CMatrix2D<int> BMP24::GetComponentPixmap ( int component )
{
	...
	switch ( component )
	{
		case Red:
			for ( auto i = 0; i < m_sInfoHeader.biHeight; i++ )
			{
				for ( auto j = 0; j < m_sInfoHeader.biWidth; j++ )
				{
					return_matrix.set ( i ,j ,m_matData.at ( i ,j )->Red );
				}
			}
			break;
		case Green:
			for ( auto i = 0; i < m_sInfoHeader.biHeight; i++ )
			{
				for ( auto j = 0; j < m_sInfoHeader.biWidth; j++ )
				{
					return_matrix.set ( i ,j ,m_matData.at ( i ,j )->Green );
				}
			}
			break;
		case  Blue:
			for ( auto i = 0; i < m_sInfoHeader.biHeight; i++ )
			{
				for ( auto j = 0; j < m_sInfoHeader.biWidth; j++ )
				{
					return_matrix.set ( i ,j ,m_matData.at ( i ,j )->Blue );
				}
			}
			break;
		default:
...
void BMP24::SaveFile ( string &filename )
{
	m_Filename = filename;
	FilenameCheck ( string ( ".bmp" ) );
	SaveFile ( );
...
CMatrix2D<rgbD> BMP24::GetPixelData ( )
{
	return m_matData;
}
void BMP24::ShowInfo ( )
...
RGBTRIPLE BMP24::GetPixel ( unsigned y ,unsigned x )
{
	...
	return RGBTRIPLE
	{
		m_matData.at ( m_sInfoHeader.biHeight - y - 1 ,x )->Blue ,
		m_matData.at ( m_sInfoHeader.biHeight - y - 1 ,x )->Green ,
		m_matData.at ( m_sInfoHeader.biHeight - y - 1 ,x )->Red
	};
}
...
void BMP24::BuildHistogramm ( BMP24 &src )
{
	vector<string> a;
...
void BMP24::BuildHistogramm ( string &filename ,CMatrix2D<int> &obj )
{
