...
void BMP8::LoadFile ( string &filename )
{
	m_Filename = filename;
	...
	try
	{
		...
		if ( m_bHeadersLoad && m_fstFileInput.is_open ( ) )
		{
			if ( m_sInfoHeader.biBitCount == bit_depth_8 && m_sInfoHeader.biCompression == BI_RGB )
			{
				...
				for ( auto i = 0; i < m_matData.size ( ); i++ )
				{
					m_matData[ i ] = pData[ i ];
				}
#ifdef _DEBUG
...
void BMP8::SaveFile ( string &filename )
{
	m_Filename = filename;
	FilenameCheck ( string ( ".bmp" ) );
	SaveFile ( );
...
CMatrix2D<rgbD> BMP8::GetPixelData ( )
{
...
CMatrix2D<int> BMP8::GetComponentPixmap ( int component )
{
